...
async function getRandomPokemon() {
  const count = (await (await fetch("https://pokeapi.co/api/v2/pokemon")).json()).count;
  const randomId = Math.floor(Math.random() * count) + 1;
  return (await fetch(`https://pokeapi.co/api/v2/pokemon/${randomId}`)).json();
}
      ...
      <div className={styles.description}>
        ...
        <div>
          <a href="https://vercel.com?utm_source=create-next-app&utm_medium=appdir-template&utm_campaign=create-next-app" target="_blank" rel="noopener noreferrer">
            By <Image src={pokemon.sprites.front_default} alt="Vercel Logo" className={styles.vercelLogo} width={100} height={24} priority />
          </a>
        </div>
      </div>
